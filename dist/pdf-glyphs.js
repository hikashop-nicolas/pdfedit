...
import { PDFArray, PDFDict, PDFName, PDFNumber, decodePDFRawStream } from "pdf-lib";
import { layoutGlyphs } from "./content-stream";
function decodedContent(page) {
    const Contents = page.node.Contents();
    const dec = (s) => decodePDFRawStream(s).decode();
    ...
    for (const [key, ref] of fontDict.entries()) {
        ...
        try {
            ...
            if (sub === "/Type0") {
                ...
                if (W instanceof PDFArray) {
                    ...
                    while (i < W.size()) {
                        ...
                        if (nx instanceof PDFArray) {
                            for (let j = 0; j < nx.size(); j++)
                                wm.set(c + j, num(ctx.lookup(nx.get(j))) ?? dw);
                            i += 2;
                        }
                        else {
                            const cl = num(nx);
                ...
                const arr = [];
                if (Wd instanceof PDFArray)
                    for (let i = 0; i < Wd.size(); i++)
                        arr.push(num(ctx.lookup(Wd.get(i))) ?? 0);
...
export function pageGlyphs(pdf, pageIndex) {
