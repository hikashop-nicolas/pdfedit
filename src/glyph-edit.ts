...
export interface Anchor {
  fontRes: string; // original page font resource, e.g. "F3"
  hex: string; // the original byte code, e.g. "0005"
...
export interface BlockGeom {
  x: number; // left edge (user space)
  firstBaseline: number; // baseline y of the first line (user space)
  lineHeight: number;
  width: number;
  align: "left" | "center" | "right" | "justify";
  size: number; // dominant size, for new text
}

export type DrawSeg =
  | { kind: "glyph"; fontRes: string; size: number; color: RGB; x: number; y: number; hex: string }
  | { kind: "text"; text: string; size: number; color: RGB; x: number; y: number };
...
export function lcsMatch(a: string, b: string): Int32Array {
  ...
  if (n === 0 || m === 0 || n * m > 4_000_000) return res;
 ...
 */
export function planEditedBlock(
  origText: string,
  anchors: (Anchor | null)[],
  editedText: string,
  geom: BlockGeom,
  ...
  const perChar: Array<{ ch: string; anchor: Anchor | null }> = [];
  {
  ...
  const lines: Array<{ ch: string; anchor: Anchor | null }[]> = [[]];
  for (const c of perChar) {
    if (c.ch === "\n") lines.push([]);
    else lines[lines.length - 1]!.push(c);
  }

  const widthOf = (c: { ch: string; anchor: Anchor | null }): number =>
    c.anchor ? c.anchor.width : measure(c.ch, geom.size);
  ...
  lines.forEach((line, li) => {
    ...
    let x = geom.x;
    if (geom.align === "center") x = geom.x + (geom.width - lineW) / 2;
    else if (geom.align === "right") x = geom.x + geom.width - lineW;
    ...
    while (i < line.length) {
      ...
      if (c.anchor) {
        ...
        while (i < line.length) {
          ...
          if (!a || a.fontRes !== a0.fontRes || a.size !== a0.size || !rgbEq(a.color, a0.color)) break;
          hex += a.hex;
          x += a.width;
          i++;
        }
