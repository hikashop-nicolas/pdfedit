...
export function createPdfEditor(container: HTMLElement, bytes: Uint8Array, options: PdfEditorOptions = {}): PdfEditor {
  ...
  async function insertImage(file: File): Promise<void> {
    ...
    const url = URL.createObjectURL(new Blob([bytesImg as BlobPart], { type: file.type }));
    img.src = url;
    img.style.left = "40px";
    img.style.top = "40px";
    ...
    makeDraggable(img);
    const rec: ImageItem = { page: target.index, bytes: bytesImg, mime: file.type, xPdf: 0, yPdf: 0, wPdf: 0, hPdf: 0, el: img };
    img.addEventListener("load", () => updateImageRect(rec, target.viewport), { once: true });
    images.push(rec);
    change();
  }

  function makeDraggable(img: HTMLElement): void {
    img.addEventListener("pointerdown", (e) => {
      ...
      const up = () => {
        ...
        if (rec && pg) updateImageRect(rec, pg.viewport);
        change();
      };
